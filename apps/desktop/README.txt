AIGC-Proof Workbench 0.7.0 Preview
==================================
...
协议与能力边界
--------------
本工作台使用公开 Rust proof-core / proof-schema 处理 AIGC-Proof 0.4.0，并兼容 0.3.0
签名包及未签名的 0.2.0。它验证证明包内部 Manifest、资产摘要和事件哈希链是否自洽，
验证 Ed25519 创建者签名，并可用显式导入的信任快照离线验证 RFC 3161 时间戳；图片
核对只判断文件是否与有效包内声明为 output 的资产字节完全一致。
...
Workbench 0.7.0 使用 ProofHostApi 1.6.0 / native API 1.5.0，并在开放证明操作前核对 engine、
protocol 0.4.0 及 0.2.0/0.3.0 兼容性、能力、执行事实和运行上限。界面显示的路径仅用于辨认，真正路径权限由
Main 内的 opaque 引用持有。任务按一项运行、最多十六项排队；排队任务可取消，已进入
原子 Rust 操作的任务只能如实标为 cancel_requested。Utility 异常会使当前任务失败且
不重放，后续任务使用新的兼容 Utility。当前没有 AIGCStudio 接入、外部身份认证、C2PA、
Rights Protection、官方服务、上传或安全中断运行中原子 Rust 操作的能力；RFC 3161 是唯一
...
已知限制
--------
- 证明包支持 0.4.0 创建者签名与可选 RFC 3161 时间戳，但应用 EXE 尚无发布者代码签名，Windows 可能显示未知发布者提示。
