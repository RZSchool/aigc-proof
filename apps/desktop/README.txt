...
检查并记录摘要绑定观察；还可对用户显式导入的官方身份声明、签发者信任快照和签名状态
...
C2PA：可选；媒体绑定、C2PA 签名者信任与 C2PA 时间戳信任分别报告
...
普通验证始终离线；只有用户点击“请求可信时间”并确认披露时，Main 才向已导入配置中
...
Workbench 1.0.0 使用 ProofHostApi 2.0.0 / native API 2.0.0，并在开放证明操作前核对 engine、
