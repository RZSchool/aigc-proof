...
协议与能力边界
--------------
本工作台使用公开 Rust proof-core / proof-schema 处理 AIGC-Proof 1.0.0，并兼容 0.5.0/0.4.0/0.3.0
签名包及未签名的 0.2.0。它验证证明包内部 Manifest、资产摘要和事件哈希链是否自洽，
验证 Ed25519 创建者签名，并可用显式导入的信任快照离线验证 RFC 3161 时间戳；可对
JPEG/PNG/WebP 内嵌 Content Credentials 或显式选择的本地 .c2pa sidecar 做离线 C2PA 2.2
检查；即使没有信任配置也会报告有界的密码学/结构有效性，显式信任快照只用于独立评估
签名者与时间戳信任，记录摘要绑定观察时仍会固定这些快照摘要。还可对用户显式导入的官方身份声明、签发者信任快照和签名状态
快照执行完全离线的严格验证。图片核对只判断文件是否与有效包内声明为 output 的资产字节完全一致。

创建者身份：显示名称是签名者自我声明，未经过实名或外部身份验证
数字签名：Ed25519 COSE_Sign1；有效性、本地信任和内部完整性分别报告
可信时间：可选；仅在用户显式请求并通过所导入 TSA 信任快照验证后报告 valid_trusted
C2PA：可选；无信任配置时有效结果报告 valid_untrusted，媒体绑定、签名者信任与时间戳信任分别报告
官方身份：可选；声明签名、显式签发者信任、创建者公钥/用途/时间绑定与状态分别报告
原创性：未评估

它不是事实真实性证明、版权登记、公证、作者身份认证、权属证明、原创认证或官方验证。

隐私与本地状态
--------------
程序不使用云服务，不上传文件，不包含账号、遥测、云 API、远程字体或 CDN 运行时。
普通验证始终离线；RFC 3161 与 C2PA 管理位于默认折叠的“高级：外部信任与内容凭证”。
只有用户点击“请求可信时间”并确认端点、摘要和策略披露时，Main 才向已导入配置中
唯一的 HTTPS TSA 端点发送创建者签名字节的 SHA-256 摘要、随机 128 位 nonce 与策略。
TSA 可能是受账户、额度、费用或可用性限制的第三方服务；本地时间、网页时钟、NTP/NTS
或通用 UTC 服务都不能替代签名的 RFC 3161 令牌。
生成适配器只连接本机 127.0.0.1:8188，不接受远程地址、凭据或重定向。
C2PA 检查不联网，不自动查找远程清单、软绑定、OCSP 或信任根；C2PA 信任配置只在
当前运行会话中保存。
官方身份验证同样不联网，只读取用户显式选择的不超过 64 KiB 的本地声明、信任与状态文件；
它不调用私有服务、不获取最新状态，也不接触证明材料、账号或私钥。
...
Workbench 1.1.0 使用 ProofHostApi 2.1.0 / native API 2.1.0，并在开放证明操作前核对 engine、
protocol 1.0.0 及 0.2.0/0.3.0/0.4.0/0.5.0 兼容性、能力、执行事实和运行上限。界面显示的路径仅用于辨认，真正路径权限由
Main 内的 opaque 引用持有。任务按一项运行、最多十六项排队；排队任务可取消，已进入
原子 Rust 操作的任务只能如实标为 cancel_requested。Utility 异常会使当前任务失败且
不重放，后续任务使用新的兼容 Utility。当前没有 AIGCStudio 接入、在线身份认证、生产 C2PA
证书签名、Rights Protection、官方服务调用、上传或安全中断运行中原子 Rust 操作的能力；RFC 3161 是唯一
受控出站网络功能，且导入的 TSA 信任快照只保存在当前运行会话，不写入 SQLite。
...
已知限制
--------
- 证明包支持 1.0.0 创建者签名、可选 RFC 3161 时间戳、可选 C2PA 观察和独立官方身份状态，但应用 EXE 尚无发布者代码签名，Windows 可能显示未知发布者提示。
