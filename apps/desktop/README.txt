AIGC-Proof Workbench 0.4.0 Preview
==================================
...
Workbench 0.4.0 使用 ProofHostApi/native API 1.2.0，并在开放证明操作前核对 engine、
...
证据事件，然后封装、立即验证并保存报告。选择 digest-only 时 prompt 原文不写入 SQLite 或
证据，重启后也不能恢复尚未运行的原文。
...
单页操作
--------
所有流程都在同一可滚动页面：工作区、五种资产角色、本地创作与自动证明、手动事件、
封装、验证/元数据检查/报告、任务历史与诊断。后续步骤不会被菜单或标签页隐藏；条件
不足时会显示前置提示。
