...
use proof_core::{
    ...
    add_asset, canonical_json, init_workspace, record_event, seal_workspace, verify_package,
};
...
fn manifest_event_and_chain_tampering_are_rejected() {
    ...
    for (filename, mutation, expected) in [
        ("manifest.aigcproof", "manifest-root", "EVENT_ROOT_HASH_MISMATCH"),
        ("payload.aigcproof", "event-payload", "EVENT_HASH_MISMATCH"),
        ("previous.aigcproof", "event-previous", "EVENT_PREVIOUS_HASH_MISMATCH"),
...
fn duplicate_manifest_and_case_conflicts_are_rejected() {
    ...
    assert_report_code(&duplicate, "MANIFEST_DUPLICATE");
...
fn all_archive_limits_are_enforced() {
    ...
    write_simple_zip(
        &package,
        &[("manifest.json", &[b'0'; 4096]), ("events.json", &[b'0'; 4096])],
    );
...
fn malformed_json_zip_spec_and_missing_asset_are_rejected() {
    ...
    for (filename, mutation, expected) in [
        ...
        ("spec.aigcproof", "manifest-spec", "UNSUPPORTED_SPEC_VERSION"),
        ("missing-asset.aigcproof", "manifest-missing-asset", "ASSET_MISSING"),
    ] {
        let destination = temp.path().join(filename);
        rewrite_zip(&source, &destination, |name, bytes| {
            mutate_json_entry(name, bytes, mutation)
...
fn mutate_json_entry(name: &str, bytes: Vec<u8>, mutation: &str) -> Vec<u8> {
    match (name, mutation) {
        ...
        ("manifest.json", "manifest-root") => {
            mutate_value(bytes, |value| value["event_chain"]["root_hash"] = json!("0".repeat(64)))
        }
        ...
        ("events.json", "event-payload") => {
            mutate_value(bytes, |value| value[0]["payload"] = json!({"changed": true}))
        }
